...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, sender_info: str, list_of_ids: list, id_of_connection: int):
        await websocket.accept()
        self.connections[id_of_connection] = websocket
        await self.broadcast(f"Client {sender_info} joined the chat.", list_of_ids)
